...
// Additional global variables for battery update handling
static volatile bool battery_update_pending = false;
static volatile uint8_t pending_battery_level = 0;
...
void get_rgb_for_battery_level(battery_level_t level, uint32_t *red, uint32_t *green, uint32_t *blue) {
    ...
    switch (level) {
        ...
        case BATTERY_LEVEL_YELLOW:
            ...
            *green = 191 * LED_BRIGHTNESS_FACTOR;
            *blue = 0 * LED_BRIGHTNESS_FACTOR;
            break;
...
void hidh_callback(void *handler_args, esp_event_base_t base, int32_t id, void *event_data)
{
    ...
    case ESP_HIDH_BATTERY_EVENT:
    {
        ...
        // Set update flag for battery change, handle in task to avoid blocking the callback
        pending_battery_level = param->battery.level;
...
// Task to handle LED updates from button press and battery changes
void led_button_task(void *pvParameters)
{
    while (1) {
        // Check the trigger flag for button press
        ...
        else if (battery_update_pending && led_initialized) {
            // Clear flag with critical section to prevent race condition
            portDISABLE_INTERRUPTS();
            battery_update_pending = false;
            uint8_t battery_level = pending_battery_level;
            portENABLE_INTERRUPTS();
            
            // Handle battery change in the task context
            handle_battery_change(battery_level);
        }
